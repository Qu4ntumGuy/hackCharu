...
def create_room(name):
    ...
    db.reference("rooms").child(str(room_id)).child("players").set({"name": f'{name}', "admin": True})
    # create room
def join_room(room_id , name):
    # join room
    room_exist = db.reference("rooms").child(str(room_id)).child("players").get()
    print(room_exist)
    if room_exist is None:
        print("Room does not exist")
        return
    else:
        db.reference("rooms").child(str(room_id)).child("players").child(str(name)).set({"name": f'{name}', "admin": False})
        print("Joined room")
        return
